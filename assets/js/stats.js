...
async function getJsonEvents(){
    ...
    let allEvents = eventsApiJson.events
    let pastEvents = eventsApiJson.events.filter(event => event.assistance)
    let upcomingEvents = eventsApiJson.events.filter(event => event.estimate)
